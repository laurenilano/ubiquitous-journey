...
sftp_folder <- paste0("fake data")
employee_survey_file <- paste0(sftp_folder, "/employee_survey_fake.sav")
...
download_link(
  link = "https://drive.google.com/uc?export=download&id=1kbD0mpWiNmQ2VWLKlC_mnXW26VeC8DrS",
...
# download_file(
#   path = system.file("employee_pdf_test.pdf", package = "downloadthis"),
#   output_name = "Employee PDF File",
#   button_label = "Download PDF file",
#   button_type = "danger",
#   has_icon = TRUE,
#   icon = "fa fa-save",
...
# Campus Climate
df_campus_climate_pos <- employee_survey %>%
  select(responseid, climate_rating) %>%
  pivot_longer(cols = climate_rating, 
               names_to = "var",
               values_to = "response") %>%
  mutate(var = "campus climate") %>%
  filter(!is.na(response)) %>%
  mutate(response = as_factor(response, levels = "labels")) %>%
  mutate(positive = ifelse(response %in% c("Very positive", "Somewhat positive"), 1, 0))

climate_positive <- df_campus_climate_pos %>%
  count(positive) %>%
  mutate(total = sum(n),
         pct = n/sum(n)) %>%
  filter(positive == 1) %>%
  mutate(construct = "Campus Climate Overall Rating") %>%
...
# Racial tension
df_racial_tension <- employee_survey %>%
  ...
  select(responseid, racialtension2) 

no_racial_tension <- df_racial_tension %>% 
  count(racialtension2) %>%
  mutate(total = sum(n),
         pct = n/sum(n)) %>%
  filter(racialtension2 == "No") %>%
  mutate(construct = "Absence of Racial Tension") %>%
  select(construct, n, total, pct)
...
constructs <- rbind(emp_belong_coll_positive, emp_belong_dept_positive, climate_positive, no_racial_tension, physical_safety_positive, emp_opp_positive, leadership_positive) %>% 
  mutate(order = case_when(construct == "Campus Climate Overall Rating" ~ 1,
                           construct == "Employee Belonging - College" ~ 2, 
                           construct == "Employee Belonging - Department" ~ 3,
                           construct == "Fair and Equal Treatment" ~ 4,
                           construct == "Absence of Racial Tension" ~ 5,
                           construct == "Physical Safety" ~ 6,
                           construct == "Leadership Effectiveness - Constituency Group" ~ 7,
                           construct == "Leadership Effectiveness - Department/Area" ~ 8,
                           construct == "Leadership Effectiveness - Executive Leadership Team" ~ 9,
                           construct == "Leadership Effectiveness - Board of Trustees" ~ 10)) %>%
  arrange(desc(order)) %>%
  mutate(construct = factor(construct, levels = c("Campus Climate Overall Rating",
                                                  "Employee Belonging - College",
                                                  "Employee Belonging - Department",
                                                  "Fair and Equal Treatment",
                                                  "Physical Safety",
                                                  "Absence of Racial Tension",
...
gglikert(employee_survey, 
  ...
  scale_x_continuous(labels = function(x) paste0(abs(x)*100, "%"), limits = c(0, 1)) +
...
microaggressions_items_all %>%
  ...
  geom_text(data = microaggressions_items_all %>% filter(var_label == "Selected any of the above"),
            aes(label = pct_neat, hjust = -0.01), color = "white", fontface = "bold") +
  geom_text(data = microaggressions_items_all %>% filter(var_label != "Selected any of the above"),
            aes(label = pct_neat, hjust = -0.01), color = "black", fontface = "bold") +
...
experienced_bipoc %>%
  ...
  geom_text(aes(label = pct_neat, hjust = -0.1), 
  ...
  # facet_wrap(~group) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 35)) +
  scale_fill_manual(values = c("#32c0d2", "#035157")) +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1)) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 45)) +
  xlab("") +
  ylab("") +
